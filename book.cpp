...
void Book::updateFileList(const PDU *pdu)
{
    ...
    for(int i=0;i<iCount;i++){
        ...
        qDebug()<<pFileInfo->caFileName<<pFileInfo->FileType;
...
void Book::createDir()
{
    ...
    if(!strNewDir.isEmpty()){
        if(strNewDir.size()>32){
            QMessageBox::warning(this,"新建文件夹","文件夹名字长度不能超过32位");
        }
        else{
...
{
    QString strDirName = index.data().toString();//接收文件夹的名字
    // qDebug()<<strDirName;
    m_strEnterDir = strDirName;
    QString strCurPath = TcpClient::getInstance().curPath();//获得当前的路径
    PDU *pdu = mkPDU(strCurPath.size()+1);
...
void Book::returnPre()
{
    ...
    if(strCurPath==strRootPath){
        QMessageBox::warning(this,"返回","返回失败：已经在根目录下");
...
void Book::uploadFile()
{
    ...
    if(!m_strUploadFilePath.isEmpty()){
        ...
        QString strFileName = m_strUploadFilePath.right(m_strUploadFilePath.size()-index-1);//提取最后一个/之后的文件名称
        // qDebug()<<strFileName;
        QFile file(m_strUploadFilePath);
        qint64 fileSize = file.size();//获得文件大小
        PDU *pdu = mkPDU(strCurPath.size()+1);
        pdu->uiMsgType = ENUM_MSG_TYPE_UPDATE_FILE_REQUEST;
        memcpy(pdu->caMsg,strCurPath.toStdString().c_str(),strCurPath.size());
        ...
        QMessageBox::warning(this,"上传文件","上传的文件名不能为空");
    }
}
...
void Book::uploadFileData()
{
    ...
    while(true){
        ...
        if(ret > 0 && ret <= 4096){
            TcpClient::getInstance().getTcpSocaket().write(pBuffer,ret);
        }
        else if(0 == ret){
            break;
        }else{
            QMessageBox::warning(this,"上传文件","上传文件事变：读文件失败");
            break;
...
void Book::moveFile()
{
    ...
    if(NULL !=pCurItem){
        ...
        QString strCurPath = TcpClient::getInstance().curPath();
        m_strMoveFilePath =strCurPath+"/"+m_strMoveFileName;//获得要移动的文件的路径
...
void Book::selectDestDir()
{
    ...
    if(NULL !=pCurItem){
        ...
        QString strCurPath = TcpClient::getInstance().curPath();
        m_strDestDir =strCurPath+"/"+strDestDir;//获得要移动的文件的目标路径
