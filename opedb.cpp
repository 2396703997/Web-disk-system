...
void OpeDB::init()
{
    ...
    m_db.setDatabaseName("C:\\Users\\Administrator\\Documents\\TcpServer\\cloud.db");
    if(m_db.open()){
        QSqlQuery query;
...
QStringList OpeDB::handleAllOnline()
{
    ...
    result.clear();
    while(query.next()){
        result.append(query.value(0).toString());//获取查询的名字内容，并将其转换为字符串
    }
    return result;
}
...
int OpeDB::handleAddFriend(const char *pername, const char *name)
{
    ...
    QString data = QString("select * from friend where "
                           ...
                           "and friendId=(select id from usrInfo where name = \'%4\'))").arg(name).arg(pername).arg(pername).arg(name);
    qDebug()<<data;
    QSqlQuery query;//查询语句
    query.exec(data);
    if(query.next()){
...
QStringList OpeDB::handleFlushFriend(const char *name)
{
    ...
    QString data=QString("select name from usrInfo where id in (select id from friend where friendId =(select id from usrInfo where name =\'%1\')) and online = 1;").arg(name);
    QSqlQuery query;
    query.exec(data);
    while(query.next()){
    ...
    // query.clear();
    data=QString("select name from usrInfo where id =(select friendId from friend where id =(select id from usrInfo where name =\'%1\')) and online = 1;").arg(name);
